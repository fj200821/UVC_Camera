...
//    }
    ...
    vd->fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    //vd->fmt.fmt.pix.pixelformat = vd->fmtack.fmt.pix.pixelformat;
    vd->fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_MJPEG;
    ...
    if (vd->fps != -1)
    {
        ...
        vd->frameint.parm.capture.timeperframe.denominator = 25;
...
static int dataProcess(v4l2_dev_t* vd,const void* p,int len ,struct timeval timestamp)
{
    ...
    if(pdata[0]==0xFF&&pdata[1]==0x55&&pdata[2]==0xFF&&pdata[3]==0xAA){
